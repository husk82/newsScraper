...
function SimilarTitles() {
  ...
  const [listOfPosts, setListOfPosts] = useState<
    { similarity: number; title1: string; title2: string }[]
  >([]);

  const fetchPosts = async () => {
    try {
      const res = await axios.get("http://localhost:3000/");
      setListOfPosts(res.data.similarTitles);
      ...
      <div className="container mx-auto p-4">
        ...
        <div className="grid gap-4 grid-cols-2">
          {listOfPosts.map((item, index) => (
            <div className="flex bg-white p-2 rounded shadow-sm" key={index}>
              <div className="mr-4">
              ...
              <div>
                <h2 className="text-lg font-semibold text-red-600">CNN</h2>
